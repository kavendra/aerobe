       ...
       <div class="container">
           ...
           <div class="left-tab">
             <ul>
               ...
               @foreach($categories as $category)
                 @if($category->knowledgeHubs->count())
                  <li><button type="button" class="btn academy-list" data-category="{{ $category->id }}">{{ $category->label }} ({{ $category->knowledgeHubs->count() }})</button></li>
                 @endif
               @endforeach
             </ul>
           </div>
         ...
         <div class="items-container grid-view active" id="post-container">
            @foreach($aerobeAcademics as $aerobeAcademy)
               @include('front.knowledge-hub.item', ['academics' => [$aerobeAcademy]])
            @endforeach
         </div>
