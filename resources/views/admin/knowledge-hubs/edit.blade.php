...
@endsection
    ...
    @section('content')
        <form method="post" class="needs-validation" novalidate action="{{ route('admin.knowledge-hub.update', $knowledgeHub->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-lg-12">
                            ...
                            <div class="p-4">
                                <div class="d-flex align-items-center">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h5 class="font-size-16 mb-1">Edit Knowledge Hub Information  <a href="{{ route('admin.knowledge-hub.index') }}" style="float:right"><button type="button" class="btn btn-outline-primary waves-effect waves-light" fdprocessedid="ub8ltb">Knowledge Hub Listing</button></a></h5>
                                ...
                                <div class="row col-lg-12">
                                            ...
                                            <select id="input9" class="form-select" name="category_id">
                                                ...
                                                @if($categories->count() > 0)
                                                    @foreach($categories as $category)
                                                        <option value="{{ $category->id }}" @if($knowledgeHub->category_id == $category->id) selected @endif>{{ $category->label }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-lg-12">
                                        <div class="mb-3">
                                            ...
                                            <textarea type="text" class="form-control" name="short_description" placeholder="Enter Short Description" id="short_description">{{ $knowledgeHub->short_description }}</textarea>
                                        </div>
                                    </div>  
                                                    ...
                                                    @foreach($countries as $country)
                                                        <div class="col-lg-2 col-md-4 col-sm-6">
                                                            <div class="form-check form-check-right mb-3">
                                                                <input class="form-check-input" type="checkbox" name="country_id[]" id="country-{{ $country->id }}" value="{{ $country->id }}" @if(is_array($knowledgeHub->country_id) && in_array($country->id, $knowledgeHub->country_id))checked @endif >
                                    ...
                                    <div class="col-lg-6 mb-3">
                                        ...
                                        <select id="input9" class="form-select" name="status">
                                            <option value="1" @if($knowledgeHub->status == 1) selected @endif>Active</option>
                                            <option value="0" @if($knowledgeHub->status == 0) selected @endif>Inactive</option>
                                        </select>
                                    </div>
